...
1.  **Sensors** capture raw audio and video data.
2.  **Logic Engine** processes this data into features and checks for triggers.
...
5.  **Intervention Engine** executes actions (TTS, sounds) if needed.
...
*   **Payload Construction**: Bundles sensor metrics, the latest video frame (Base64), raw audio snippet, and current state context into a JSON payload for the LMM.
...
*   **Prioritization**:
    *   **System Triggers**: Specific logic (e.g., "Doom Scroll" detection via persistent visual context) can override LMM suggestions.
