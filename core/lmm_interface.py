...
class LMMInterface:
    ...
    def process_data(self, video_data=None, audio_data=None, user_context=None) -> Optional[LMMResponse]:
        ...
        try:
            ...
            # Log Token Usage
            self._log_info(f"LMM Success. Latency: {latency_ms:.2f}ms | Tokens: Prompt={usage.get('prompt_tokens', '?')}, Completion={usage.get('completion_tokens', '?')}, Total={usage.get('total_tokens', '?')}")
