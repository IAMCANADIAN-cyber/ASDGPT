    ...
    def update(self) -> None:
        ...
        if current_mode in ["active", "dnd"]:
            ...
            if self.input_tracking_enabled: # Only trust idle time if tracking is working
                ...
                # Track speech duration (with grace period for short gaps)
                if is_speech:
                    self.last_speech_time = current_time

                # Reset timer if we haven't spoken for longer than grace period
                if (current_time - self.last_speech_time) > speech_grace_period:
                    self.continuous_speech_start_time = 0

                # Start timer if speaking and face present, and timer not already running
