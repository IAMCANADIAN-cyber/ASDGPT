...
class InterventionEngine:
    ...
    def start_intervention(self, intervention_details: Dict[str, Any], category: str = "default") -> bool:
        ...
        with self._lock:
            # --- Escalation Logic (Check First) ---
            if intervention_id:
                 # Look back in history for last occurrence of this ID
                 ...
                 if last_occurrence:
                     ...
                     if time_delta < self.escalation_window:
                         ...
                         if time_delta > nag_interval:
                             # Valid Escalation (past nag interval) -> Escalate Tier & Bypass standard cooldowns
                             current_tier = execution_details.get("tier", 1)
                             last_tier = last_occurrence.get("tier", 1)

                             # Monotonic escalation: If new tier is not higher, force increment
                             if current_tier <= last_tier:
                                 new_tier = last_tier + 1
                                 execution_details["tier"] = new_tier
                                 if logger: logger.log_info(f"Escalating intervention '{intervention_id}' to Tier {new_tier} (Time Delta: {time_delta:.1f}s)")

                             escalation_active = True
                         else:
                             # Too soon (spam prevention during escalation window)
                             if logger: logger.log_debug(f"Escalation suppressed: Too soon since last trigger ({time_delta:.1f}s < {nag_interval}s)")
                             return False

            # --- Standard Cooldowns (Only if NOT escalating) ---
            if not escalation_active:
                ...
                if current_time - last_cat_time < category_limit:
                    if logger: logger.log_debug(f"Intervention suppressed: Category '{category}' cooldown active ({category_limit}s).")
                    return False
                ...
                if not bypass_global and not is_system_msg:
                     if current_time - self.last_intervention_time < config.MIN_TIME_BETWEEN_INTERVENTIONS:
                        if logger: logger.log_debug(f"Intervention suppressed: Global cooldown active.")
                        return False
            ...
            if self._intervention_active.is_set():
                ...
                if new_tier > current_tier:
                    ...
                    # Note: We are inside lock. 'stop_intervention' might need lock? No, 'stop_intervention' just clears event.
                    # But 'join' would block. We handle preemption outside lock below.
                else:
                    if logger:
