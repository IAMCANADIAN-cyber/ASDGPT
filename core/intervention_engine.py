...
class InterventionEngine:
    ...
    def start_intervention(self, intervention_details: Dict[str, Any], category: str = "default") -> bool:
            ...
            if intervention_id:
                 ...
                 if last_occurrence:
                     ...
                     if time_delta < self.escalation_window:
                         # Repeated trigger! Check tier.
                         current_req_tier = execution_details.get("tier", 1)
                         if current_req_tier <= last_occurrence["tier"]:
                             # Escalate!
                             is_escalation = True
                             new_tier = last_occurrence["tier"] + 1
                             if new_tier > 3: new_tier = 3
                             execution_details["tier"] = new_tier
                             if logger: logger.log_info(f"Escalation detected: '{intervention_id}' -> Tier {new_tier}")

            # --- Cooldown Checks ---

            if is_escalation:
                # Use Escalation Nag Interval
                nag_interval = getattr(config, 'ESCALATION_NAG_INTERVAL', 15)
                # Check time since last occurrence
                if current_time - last_occurrence["timestamp"] < nag_interval:
                     if logger: logger.log_info(f"Escalation suppressed: Nag interval ({nag_interval}s) not met.")
                     return False
                # If met, we BYPASS standard cooldowns
                if logger: logger.log_info("Escalation bypassing standard cooldowns.")
        ...
        with self._lock:
            # Re-confirm state (though we are single threaded here mostly)
