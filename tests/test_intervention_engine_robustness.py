...
class TestInterventionEngineRobustness:
    @pytest.fixture(autouse=True)
    def setup_mocks(self):
        mock_config = MagicMock()
        mock_config.FEEDBACK_WINDOW_SECONDS = 5
        mock_config.MIN_TIME_BETWEEN_INTERVENTIONS = 0.1
        mock_config.FEEDBACK_SUPPRESSION_MINUTES = 60
        mock_config.SUPPRESSIONS_FILE = "suppressions.json"
        mock_config.PREFERENCES_FILE = "preferences.json"
        mock_config.SNOOZE_DURATION = 60

        modules = {'config': mock_config}

        with patch.dict(sys.modules, modules):
            if 'core.intervention_engine' in sys.modules:
                del sys.modules['core.intervention_engine']
            yield
    ...
    @pytest.fixture
    def engine(self, InterventionEngine, mock_logic_engine, mock_app):
