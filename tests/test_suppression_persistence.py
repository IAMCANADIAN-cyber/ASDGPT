...
def setup_test_env():
    ...
    # Manually save and restore config to ensure robustness against import pollution
    orig_user_data = getattr(config, 'USER_DATA_DIR', 'user_data')
    orig_suppressions = getattr(config, 'SUPPRESSIONS_FILE', 'user_data/suppressions.json')
