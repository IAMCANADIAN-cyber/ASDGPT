...
def setup_test_env():
    ...
    # Patch both the global config and the one imported in intervention_engine
    with patch('config.USER_DATA_DIR', TEST_USER_DATA_DIR), \
         patch('config.SUPPRESSIONS_FILE', TEST_SUPPRESSIONS_FILE), \
         patch('core.intervention_engine.config.USER_DATA_DIR', TEST_USER_DATA_DIR), \
         patch('core.intervention_engine.config.SUPPRESSIONS_FILE', TEST_SUPPRESSIONS_FILE):
        yield
