...
from tools.generate_timeline import parse_events, generate_markdown, process_log_file
...
class TestTimelineGeneration(unittest.TestCase):
    ...
    def test_process_log_file(self):
        ...
        # We wrote 2 lines that should generate events
        self.assertTrue(len(events) >= 2)

        trigger = next((e for e in events if e["event_type"] == "lmm_trigger"), None)
        self.assertIsNotNone(trigger)
        self.assertEqual(trigger["payload"]["reason"], "high_audio_level")

        state_update = next((e for e in events if e["event_type"] == "state_update"), None)
        self.assertIsNotNone(state_update)
        self.assertEqual(state_update["payload"]["arousal"], 60)
