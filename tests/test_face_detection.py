...
class TestFaceDetection(unittest.TestCase):
    ...
    def test_analyze_frame_with_none(self):
        ...
        # Expect default values, not empty dict
        self.assertFalse(metrics.get('face_detected', True))
        self.assertEqual(metrics.get('face_count', -1), 0)
    ...
    def test_analyze_frame_exception(self):
        ...
        # Should handle exception gracefully and return default values
        self.assertFalse(metrics.get('face_detected', True))
        self.assertEqual(metrics.get('face_count', -1), 0)
